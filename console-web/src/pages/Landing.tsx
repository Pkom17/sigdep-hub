      ...
      <section className="bg-gradient-to-b from-sigdep-50 to-white">
        <div className="mx-auto max-w-6xl px-6 py-12">
          <h1 className="text-3xl sm:text-4xl font-semibold tracking-tight">
            Tableau de bord national VIH
          </h1>
          <p className="mt-2 text-ink-muted">
            Indicateurs clés du programme national de lutte contre le sida
          </p>
          <p className="mt-4 text-xs text-ink-subtle">
            Données mises à jour quotidiennement · Source : SIGDEP-3 ·{' '}
            <span className="font-medium">
              Données partielles — {data ? formatInt(data.sitesWithData) : '…'} sites
              synchronisés sur {NATIONAL_TARGET_SITES} ciblés
            </span>
          </p>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-6xl w-full px-6 -mt-6">
        <div className="grid gap-3 grid-cols-2 lg:grid-cols-4">
          ...
          <Kpi label="Suppression virale"
               ...
               hintTone="positive" />
          <Kpi label="Couverture ARV adultes"
               value={data?.arvCoverage == null ? 'À venir' : formatPercent(data.arvCoverage)}
          ...
          <div className="h-48">
            {isLoading ? (
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={data?.activeFile ?? []} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
      ...
      <footer className="mt-12 border-t border-slate-200 bg-white">
        <div className="mx-auto max-w-6xl px-6 py-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <p className="text-xs text-ink-subtle">
            Ministère de la Santé · PNLS · 2026
          </p>
        </div>
      </footer>
    </div>
